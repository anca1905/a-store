...
if ($hitung) {
    ...
    $sql = "SELECT 
                DATE_FORMAT(DATE_SUB(p.tanggal_waktu, INTERVAL WEEKDAY(p.tanggal_waktu) DAY), '%d %b %Y') as tgl_awal,
                YEARWEEK(p.tanggal_waktu, 1) as yw,
                SUM(d.qty) as total_qty
            FROM tbl_detail_penjualan d
            JOIN tbl_penjualan p ON d.id_penjualan = p.id_penjualan
            WHERE d.kode_barang = '$kode_barang'
            GROUP BY yw, tgl_awal
            ORDER BY yw DESC
            LIMIT $n_periode";
    ...
    if ($n_existing < $n_periode) {
        ...
        $mock_data = [];
        for ($i = $needed; $i >= 1; $i--) {
            $tgl = date('d M Y', strtotime("-$i week"));
            $mock_data[] = ['tgl_awal' => $tgl, 'total_qty' => rand(15, 45)];
        }
    ...
    $hasilPeramalan = round(max(0, $L[$n-1] + $T[$n-1]));
...
<div style="background:#fff; border-radius:16px; border:1px solid var(--border-color); padding:32px; display:grid; grid-template-columns: 1fr 300px; gap:40px; margin-bottom:24px;">
    ...
    <div>
        <form method="GET" style="display:flex; flex-direction:column; gap:16px;">
            ...
            <div style="display:grid; grid-template-columns: 160px 1fr; align-items:center;">
                <label class="form-label" style="margin:0;">Nilai Alpha (α)</label>
                <input type="number" name="alpha" class="form-control" value="<?= $alpha ?>" min="0.01" max="0.99" step="0.01" style="background:var(--bg-body); border-color:transparent;">
            </div>
    ...
    <div style="border-left:1px solid var(--border-color); padding-left:40px;">
        ...
        <?php if ($hitung && isset($hasilPeramalan)): ?>
            ...
            <div style="margin-bottom:24px;">
                ...
                <div style="font-size:13px; line-height:1.5;">Diperkirakan penjualan (kebutuhan stok) pada minggu depan sebanyak <?= number_format($hasilPeramalan, 0, ',', '.') ?> unit. Akurasi model <?= max(0, $akurasi) ?>% (<?= $kualitas ?>).</div>
            </div>
...
<div id="detailPerhitungan" style="display:none; background:#fff; border:1px solid var(--border-color); border-radius:16px; overflow:hidden; animation: fadeIn 0.3s; margin-bottom:24px;">
    <div style="padding:20px; border-bottom:1px solid var(--border-color); display:flex; justify-content:space-between; align-items:center;">
        <h3 style="font-size:15px; font-weight:700;">Data Peramalan — DES (α = <?= $alpha ?>)</h3>
        ...
        <table class="data-table" style="text-align:center;">
            ...
            <tbody>
                ...
                <tr style="background:var(--primary-light); font-weight:700;">
                    ...
                    <td colspan="4" style="text-align:left; padding-left:20px;">← Hasil Peramalan</td>
                </tr>
            </tbody>
        </table>
    </div>
